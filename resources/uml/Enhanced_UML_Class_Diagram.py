...
with uml_diagram.subgraph(name='cluster_0') as view_layer:
    ...
    # View Classes
    view_layer.node('AppointmentView', '{K2434232_AppointmentView|scanner: Scanner\\lappointmentController: AppointmentController\\lpatientView: PatientView\\ldermatologistView: DermatologistView\\ltreatmentView: TreatmentView\\ltimeView: TimeView\\linvoiceView: InvoiceView\\l|showAppointmentMenu(): void\\ldisplayMenu(): void\\lgetUserChoice(): int\\lscheduleAppointment(): void\\lviewAllAppointments(): void\\lupdateAppointment(): void\\lcancelAppointment(): void\\lcompleteAppointment(): void\\lprocessInvoicePayment(): void\\lsearchAppointment(): void\\l}')

    view_layer.node('PatientView', '{K2434232_PatientView||getPatientDetails(): Patient\\l}')

    view_layer.node('DermatologistView', '{K2434232_DermatologistView|scanner: Scanner\\ldermatologistService: DermatologistService\\l|selectDermatologist(): DermatologistModel\\laddTimeSlotForDermatologist(): void\\lgetValidDateFromUser(): LocalDate\\lgetValidStartTimeFromUser(): LocalTime\\l}')

    view_layer.node('TreatmentView', '{K2434232_TreatmentView||getTreatmentDetails(): Treatment\\l}')

    view_layer.node('TimeView', '{K2434232_TimeView||getTimeDetails(dermatologist: DermatologistModel): Time\\l}')

    view_layer.node('InvoiceView', '{K2434232_InvoiceView||getInvoiceDetails(treatment: Treatment): Invoice\\l}')
...
with uml_diagram.subgraph(name='cluster_2') as repository_layer:
    ...
    # Repository Classes
    repository_layer.node('AppointmentRepository', '{K2434232_AppointmentRepository|appointments: List<Appointment>\\l|save(appointment: Appointment): void\\lfindAll(): List<Appointment>\\lfindById(appointmentId: int): Optional<Appointment>\\ldelete(appointmentId: int): boolean\\lupdate(appointment: Appointment): boolean\\l}')

    repository_layer.node('DermatologistRepository', '{K2434232_DermatologistRepository|dermatologists: List<DermatologistModel>\\l|save(dermatologist: DermatologistModel): void\\lfindAll(): List<DermatologistModel>\\lfindById(id: int): Optional<DermatologistModel>\\laddTimeSlot(dermatologist: DermatologistModel, time: Time): boolean\\l}')

    repository_layer.node('PatientRepository', '{K2434232_PatientRepository|patients: List<Patient>\\l|save(patient: Patient): void\\lfindAll(): List<Patient>\\lfindById(id: int): Optional<Patient>\\l}')
...
with uml_diagram.subgraph(name='cluster_3') as domain_layer:
    ...
    domain_layer.node('Appointment', '{K2434232_Appointment|appointmentId: int\\lpatient: Patient\\ldermatologist: DermatologistModel\\ltreatment: Treatment\\ltime: Time\\linvoice: Invoice\\lstatus: AppointmentStatus\\l|setStatus(status: AppointmentStatus): void\\lgetInvoice(): Invoice\\lgetId(): int\\l}')

    domain_layer.node('Patient', '{K2434232_Patient|patientId: int\\lname: String\\lage: int\\lcontactInfo: String\\lmedicalHistory: String\\l|getId(): int\\lgetName(): String\\lgetAge(): int\\l}')

    domain_layer.node('DermatologistModel', '{K2434232_DermatologistModel|dermatologistId: int\\lname: String\\lspecialization: String\\lavailableTimeSlots: List<Time>\\l|checkAvailability(time: Time): boolean\\laddTimeSlot(time: Time): void\\l}')

    domain_layer.node('Treatment', '{K2434232_Treatment|treatmentId: int\\lname: String\\ldescription: String\\lduration: int\\lcost: double\\l|getCost(): double\\lgetDuration(): int\\l}')

    domain_layer.node('Invoice', '{K2434232_Invoice|invoiceId: int\\ltotalAmount: double\\lamountPaid: double\\lstatus: PaymentStatus\\l|makePayment(amount: double): void\\lisPaid(): boolean\\lgetBalance(): double\\l}')

    domain_layer.node('Time', '{K2434232_Time|date: LocalDate\\lstartTime: LocalTime\\lendTime: LocalTime\\l|getDate(): LocalDate\\lgetStartTime(): LocalTime\\lgetEndTime(): LocalTime\\lisAvailable(): boolean\\l}')
